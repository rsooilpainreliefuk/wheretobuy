                ...
                var knowledge = data;    
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                            ...
                            $('#openai-answer').append('<div class="loading">Fetching answer...</div>');
                        },
                        success: function(response) {
                            $('.loading').remove();
                            if (response.choices && response.choices[0] && response.choices[0].text) {
                                $('#openai-answer').append('<div class="question-answer"><strong>Question:</strong> ' + question + '<br><strong>Answer:</strong> ' + response.choices[0].text + '</div><hr>');
                                ...
                                $('#openai-answer').append('<div class="error"><strong>Error:</strong> Unable to fetch answer.</div><hr>');
                            }
                        },
                        error: function() {
                            $('.loading').remove();
            ...
            .then(function(data) {
                ...
                var api_key = 'sk-' + 'J34zghOMevd' +'Lmh0j7UZaT3B' +'lbkFJtxrhQ' +'gxK6e17' +'m2eHrDDn';
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                            xhr.setRequestHeader('Authorization', 'Bearer ' + api_key);
                            $('#openai-answer2').append('<div class="loading">Fetching answer...</div>');
                        },
                        success: function(response) {
                            $('.loading').remove();
                            if (response.choices && response.choices[0] && response.choices[0].text) {
                                $('#openai-answer2').append('<div class="question-answer"><strong>Question:</strong> ' + question + '<br><strong>Answer:</strong> ' + response.choices[0].text + '</div><hr>');
                                ...
                                $('#openai-answer2').append('<div class="error"><strong>Error:</strong> Unable to fetch answer.</div><hr>');
                            }
                        },
                        error: function() {
                            $('.loading').remove();
